<?php 
	...
	*/
	...
	function send_payment($to,$body)	
	{	
		$subject = get_custom('email_subject');	
	...
	function get_post_by_title($page_title, $post_type ='transactions' , $output = OBJECT) {
		global $wpdb;
			$post = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE id = %s AND post_type= %s", $page_title, $post_type));
			if ( $post )
	...
	define('THEME_FULL_NAME','Tour Package');
	...
	$temp_root = get_root_directory('include/plugin/gdl-paypal.php');
	include_once($temp_root . 'include/plugin/gdl-paypal.php');	
...
function wpdocs_scripts_method() {
    wp_enqueue_script( 'fle-custom', get_stylesheet_directory_uri() . '/javascript/fle-custom.js', array( 'jquery' ) );
}
